# MusicBot v16 (HHMusic) — Agent Guide
...
- **Spotify OAuth flow** (`src/Spotify.js`): The bot runs an HTTP server on port 3000 to handle the OAuth callback. Use `http://127.0.0.1:3000/spotify-callback` (NOT `localhost` — Spotify now rejects `localhost` for new apps). The refresh token is persisted to `database/spotify_token.json` AND the env var `SPOTIFY_REFRESH_TOKEN` (for Railway persistence).
- **Spotify API change (2025)**: For apps created after April 2025, `GET /v1/playlists/{id}/tracks` returns 403. Use `GET /v1/playlists/{id}` instead — the response now has `items` as a top-level object (`{ href, items: [...] }`), and each entry uses `item` (NOT `track`) as the track data field: `entry.item || entry.track`.
...
- **Dockerfile**: `node:24-slim` base, installs system python3 + ffmpeg, `EXPOSE 8080`.
