...
- **Autoplay**: Toggle via the 🎲 button or `/autoplay` (both just set `player.autoplay = 'related'` — no genre picker). When the queue ends, `MusicPlayer.handleAutoplay` first grabs YouTube related videos from the **last played track**, falling back to a search by that track's **artist name**, then to generic `music official video` / `top songs ${currentYear}` keywords. Filters 30s–600s and blocks tutorial/podcast/etc. titles. It plays 1 candidate immediately, then tops the queue up to ~50 tracks in the background via `refillAutoplayQueue` (parallel searches, deduped by URL against the queue + `previousTracks`). If it finds nothing, it clears `currentTrack` and falls through to the normal queue-completed cleanup instead of leaving the player stuck. `previousTracks` is capped at 50 entries. (The old genre-select menu and `genres.*`/`autoplay_genre` keys are dead code; `genres.*` translations still exist in the language files unused.)